...
app.post('/campaign/:campaignId', async (c) => {
  ...
  // DEFAULT (force=false, includeBad=false): re-verify PENDING + VERIFYING + RISKY.
  //   - VERIFIED leads are SKIPPED (they passed all layers — no need to re-check)
  //   - BAD leads are SKIPPED (user explicitly requested: "any lead already marked
  //     as bad should not be reverified unless i go to the page section and
  //     request for it")
  //   - RISKY leads ARE re-verified (often got that way due to transient SMTP
  //     failures — worth a second chance)
  //   - PENDING + VERIFYING (stuck) leads are re-verified
  //
  // INCLUDE_BAD (includeBad=true, force=false): re-verify PENDING + VERIFYING +
  //   RISKY + BAD. The frontend exposes this as a checkbox "Include BAD leads"
  //   next to the Start Verification button. Use when you've fixed the SMTP
  //   provider config and want to re-check previously-BAD leads.
  //
  ...
  //   logic update). Not currently exposed in the UI — call the API directly
  //   with { force: true } in the body.
  const force = !!body.force
  const includeBad = !!body.includeBad
  const where = force
    ? { campaignId }
    : includeBad
      ? {
